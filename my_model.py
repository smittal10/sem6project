...
token_to_id_path        = './Toy/train/token2id.txt'
token_2_id =load_token_to_id(token_to_id_path)
token_frequencies_file_path = './Toy/train/frequencies.txt'
segment_sepparator = '<eos>'
train_captions = get_captions('./Toy/train/capsos.txt')
valid_captions = get_captions('./Toy/validation/capsos.txt')
caption_2_id=load_caption_2_id('./Toy/train/idtocap.txt')
valid_caption_2_id=load_caption_2_id('./Toy/validation/idtocap.txt')
id_2_imageid=load_id_2_imageid('./Toy/train/idtoimgid.txt')
valid_id_2_imageid=load_id_2_imageid('./Toy/validation/idtoimgid.txt')
img_file='/media/saloni/284C-37FC/inception_v3/train2014/COCO_train2014_'
valid_img_file='/media/saloni/284C-37FC/inception_v3/val2014/COCO_train2014_'
...
def average_cross_entropy(full_cross_entropy_node, input_node, image_node,label_node, data):
    count = 0
    ce_sum = 0
    for features, labels,img, _ in data.create_batch(valid_captions,token_2_id,valid_caption_2_id,valid_id_2_imageid,valid_img_file):
        arguments = ({input_node : features,image_node:img, label_node : labels})
...
def create_model(input_sequence,image_sequence,label_sequence, vocab_dim, hidden_dim):
     #img_embed = Dense(hidden_dim, activation=relu)(image_sequence)
     ...
     #input_seq = [(image_sequence,word_embed),splice]
     #input_seq = np.concatenate ((image_sequence,word_embed),axis=0)
     input_seq = C.splice(image_sequence,word_embed,axis=0)
    ...
    # Apply it to the input sequence. 
     latent_vector = rnn_with_latent_output(input_seq)
...
def create_inputs(vocab_dim):
    ...
    input_sequence = sequence.input(shape=vocab_dim, sequence_axis=input_seq_axis, is_sparse = True)
    image_sequence = sequence.input(shape=image_dim, sequence_axis=input_seq_axis, is_sparse = False)
    label_sequence = sequence.input(shape=vocab_dim, sequence_axis=input_seq_axis, is_sparse = True)
    
    return input_sequence,image_sequence, label_sequence
...
def train_lm():
    ...
    input_sequence,image_sequence, label_sequence = create_inputs(data.vocab_dim)
    ...
    z, cross_entropy, error = create_model(input_sequence, image_sequence,label_sequence, data.vocab_dim, hidden_dim)
    ...
    for epoch_count in range(num_epochs):
        for features, labels, img_features, token_count in data.create_batch(train_captions, token_2_id, caption_2_id,id_2_imageid,img_file):
            arguments = ({input_sequence : features,image_sequence : img_features, label_sequence : labels})
            ...
            if num_trained_samples_since_last_report >= num_samples_between_progress_report or num_trained_samples == 0:
                av_ce = average_cross_entropy(full_ce, input_sequence,image_sequence, label_sequence, data)
